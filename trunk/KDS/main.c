...
unsigned char	RFtype;		// Determines what type of encrypted signal
							// was received
...
void main ()
{
	...
	printf("KEELOQ 3 DEVELOPMENT KIT\r\n");
	Display_Toggle = 0;
...
/*
		...
		lcd_puts("   ");				// the decrypted result until the transmission is fully validated.
	   	DelayUs(200);
	    	lcd_goto(LCD_LINE_2);
...
*/
		...
		for (k=4; k>0; k--)				// Note: The LCD output of the decrypted data is intended
		{							// for testing and verification purposes, user may want to 
	      	put_byte(SEED[k-1]);		// remove the LCD result display lines and only show 
		}
		printf("\r\n");
		printf("Result: ");
	   	for (k=4; k>0; k--)
...
*/
		printf("Not a Valid Transmitter\r\n");
...
/*
			...
			lcd_goto(LCD_LINE_1);			// on LCD
	   		lcd_puts("Transmitter Not ");   
	    		...
	    		lcd_puts("    Learned! ");
*/
			printf("Transmitter Not Learned!\r\n");
			return; 
		 }
		else
		{
